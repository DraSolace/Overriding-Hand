...
Клиент-макросник c GUI на чистом питоне, для которого я сделал свой алгоритмический язык программирования **"Overscript"** <sup> (пожалуйста, не смотри как работает интерпретатор для него, или хотя бы не смейся) </sup>
...
Эти макросы создаются на собственном **Overscript**. Подробная документация с кучей примеров и разбором разных ситуаций встроена в приложение

![](https://media.giphy.com/media/IoAFDKJR8XbtO8LhtA/giphy.gif)

Макросы способы на довольно гибкую эмуляцию любых нажатий клавы\мыши\колесика\комбинаций всего перечисленного + движений курсора. Операторов и структур свойственных фукнциональному языку вроде циклов, функций, if\else нет. Гибкость заключается в разных режимах работы и директивах.

**Overscript** был умышленно сделан примитивным, но высокоуровневым. Любой пользователь может не напрягаясь накидать пару команд и получить нужный результат. А если что - местный "интерпретатор" выдаст тебе наводящую ошибку.
Создать прямо таки IDE с хайлайтами и контекстуальными подсказками мне не под силу, масенький я ещё. Но этого ***минимума*** должно хватить. А даже если имея ошибку интерпретатора на руках вы не понимаете в чем проблема - ничего не помешает свериться с документацией и продолжить написание своего макроса :)

![](https://media.giphy.com/media/wRe839uv0144kyyxaA/giphy.gif)

Есть в Overriding hand ещё много разных маленьких приспособ для пущего удобства, найдешь это всё в доках :)


## База данных! 
*ТУТ ЕСТЬ БД НО НЕТ ВСТРОЕННОЙ СУБД ДЛЯ НЕЁ*!
...
## Легаси билд
Этот репозиторий являеся "форком" старого, где есть [бранч "Built"](https://github.com/DraSolace/Overriding-Handv1.1/tree/Built). Там Overriding Hand ***уже*** собран в exe вместе с исходным кодом. Гарант работоспособности только для винды либо в wine'е.
